...
class MovieAdapter(
    ...
    override fun onBindViewHolder(holder: MovieView, position: Int) {
        ...
        for (i in 0..listGenres.size - 1) {
            if (list[position].genre_ids.contains(listGenres[i].id)) {
                if (holder.itemView.genre_text.text.length == 0)
                    ...
                    holder.itemView.genre_text.text = String.format(
                        "%s, %s",
                        holder.itemView.genre_text.text.toString(),
                        listGenres[i].name
                    )
            }
        }
        ...
        holder.itemView.genre_text.background = null
        holder.itemView.movie_name.background = null
    }
